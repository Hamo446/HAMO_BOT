...
client.on('message', async message => {
    try {
        ...
        const senderName = contact.pushname || contact.name || 'صديق';
        const senderNumber = contact.number.replace('@c.us', '');
        
        console.log(`📩 ${senderName}: ${message.body}`);
        
        // تجاهل المجموعات
        if (chat.isGroup && !message.body.toLowerCase().includes('هامو')) {
            return;
        }
...
app.get('/logs', (req, res) => {
    ...
    <body>
        ...
        <div>
        🔧 Logs متاحة فقط في Koyeb Dashboard
        🔗 اذهب إلى Koyeb → App → Logs
        📱 لمشاهدة QR code
        </div>
        <br>
        <a href="/" style="color: #ff0064;">← الرجوع للصفحة الرئيسية</a>
    </body>
    </html>
    `);
});
